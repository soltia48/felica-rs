...
fn load_keys_from_jsonl(path: &str) -> Result<SystemKeys, Box<dyn Error>> {
    ...
    for (index, line_result) in reader.lines().enumerate() {
        ...
        let Some(node) = parse_hex_u16_key_field(line_num, "node", &record.node) else {
            continue;
        };
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let key_store = load_keys_from_jsonl(&keys_path)?;
...
fn resolve_auth_keys(
    ...
    derive_service_keys(card_keys, areas, services).ok_or_else(|| {
        make_input_error(format!(
            "Missing required node keys for authentication (IDm: {}); check keys.jsonl",
            idm_hex
        ))
    })
}
